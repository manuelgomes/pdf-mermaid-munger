...
class MarkdownMermaidMunger:
    ...
    def materialize(self, src: str, root: str):
        ...
        tgt = NamedTemporaryFile(
            dir=root, prefix=origin.name, suffix=origin.suffix, delete=False
        )
    ...
    def munge(self) -> str:
        ...
        try:
            ...
            source = shutil.copyfile(
                src=self.path, dst=Path(dir).joinpath(self.path.name)
            )
            temp_md = NamedTemporaryFile(dir=dir, delete=False, suffix=".md")
            self.mermaid2png(source=source, temp_md=temp_md)

            soup = BeautifulSoup(
                Markdown().convert(Path(temp_md.name).read_text()), features="lxml"
            )
